...
if (!hz) {
  ...
  assert.match(main, /fragmentSystem.update\([^;]+;\s*if \(!isRuntimeActive\(\)\) return;\s*portal.update\([^;]+;\s*exitPortal.update\([^;]+;\s*if \(!isRuntimeActive\(\)\) return;\s*mobileAsteroid.update/);
